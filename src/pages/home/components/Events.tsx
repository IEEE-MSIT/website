            ...
            <div className="flex gap-6 pb-4 w-max">
              {events.map((event, index) => (
                <div key={index} className={`bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 w-[580px] flex-shrink-0`}>
                  <div className="flex">
                    <div className={`bg-gradient-to-br ${event.featured ? 'from-[#C84C31] to-[#A7441C]' : 'from-[#A7B77F] to-[#8a9a5b]'} text-white p-6 flex flex-col items-center justify-center min-w-[120px]`}>
                    ...
                    <div className="flex-1 p-6 flex flex-col justify-between">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-sm font-semibold text-[#C84C31]">{event.society}</span>
                          <span className="text-xs text-gray-500">{event.prize}</span>
                        </div>
                        <button className={`px-5 py-2.5 rounded-lg text-sm font-medium transition-colors shadow-md hover:shadow-lg ${event.featured ? 'bg-[#C84C31] text-white hover:bg-[#A7441C]' : 'border-2 border-[#A7B77F] text-[#A7B77F] hover:bg-[#A7B77F] hover:text-white'}`}>
                          {event.buttonText}
                        </button>
                      </div>
                    </div>
              ...
              <div className="flex gap-4 pb-4 w-max">
                {events.map((event, index) => (
                  <div key={index} className="bg-white border border-gray-200 rounded-xl overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 w-[320px] flex-shrink-0">
                    <div className={`h-40 bg-gradient-to-br ${event.featured ? 'from-[#C84C31] to-[#A7441C]' : 'from-[#A7B77F] to-[#8a9a5b]'} text-white p-4 flex flex-col justify-center relative`}>
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex flex-col">
                          <span className="text-xs font-semibold text-[#C84C31]">{event.society}</span>
                          <span className="text-xs text-gray-500">{event.prize}</span>
                        </div>
                        <button className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${event.featured ? 'bg-[#C84C31] text-white hover:bg-[#A7441C]' : 'border-2 border-[#A7B77F] text-[#A7B77F] hover:bg-[#A7B77F] hover:text-white'}`}>
                          {event.buttonText}
                        </button>
                      </div>
                    </div>
